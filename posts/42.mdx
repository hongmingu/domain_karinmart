...
예를 들어 index 액션은 모든 Post를 가져오고 show 액션은 특정 Post를 가져오는 등의 작업을 처리해야 하자.
...
이 코드에서 params[:id]를 통해 URL의 id 파라미터를 읽는다.
